package uniroma3.siw.videogames.authentication;

import static uniroma3.siw.videogames.model.Credentials.ADMIN_ROLE;
import static uniroma3.siw.videogames.model.Credentials.DEFAULT_ROLE;
...
public class AuthConfiguration {
	...
	protected SecurityFilterChain configure(final HttpSecurity httpSecurity) throws Exception {
		httpSecurity.csrf().and().cors().disable().authorizeHttpRequests()
				...
				.requestMatchers(HttpMethod.GET, "/", "/index", "/error/**", "/videogiochi", "/videogiochi/**",
						"/login", "/register", "/css/**", "/images/**", "favicon.ico")
				.permitAll().requestMatchers(HttpMethod.POST, "/register", "/login")
				.permitAll()

				// Solo ADMIN_ROLE
				.requestMatchers("/operatore/**").hasAuthority(ADMIN_ROLE)
				...
				.successHandler((request, response, authentication) -> {
					...
					if (isAdmin) {
						...
						response.sendRedirect(idUtente != null ? "/operatore/videogiochi" : "/operatore/" + idUtente);
					} else {
						// Se Utente, redirect utente (sostituisci idUtente)
						response.sendRedirect(idUtente != null ? "/utente/videogiochi" : "/utente/" + idUtente);
					}
